...
def parse_args():
    ...
    parser.add_argument('--model_path', default='/home/fang/data4t/plant-segmegmentation-models/model_output/pepper_pepper/Iteration/',
                        help='model path')
    parser.add_argument('--valResult_path', default='/home/fang/data4t/plant-segmegmentation-models/val_output/pepper_pepper/Iteration/',
                        help='model path')
    parser.add_argument('--model_name', default='potsdam_ResNet_UTNet_woDS',
                        help='model name')
    parser.add_argument('--test_path', default='/home/fang/datasets/pepper/pepper/Test/',     #image  label
...
def main():
        ...
        with torch.no_grad():
            ...
            for input, target, meta in tqdm(val_loader, total=len(val_loader)):
                ...
                for i in range(len(output)):
                    for c in range(config['num_classes']):
                        save_path = args.test_path + each_model + '/' + str(c) + '/' + \
                                    meta['img_id'][i].split('/')[-1][:-4] + '.jpg'
    ...
    # print("Hausdorff distance test: {0}".format(hausdorff_distance(X, Y, distance="cosine")))
        ...
        print('IoU: %.4f, F1: %.4f, accuracy: %.4f, recall: %.4f, SPE: %.4f, pre: %.4f' % (avg_meter.avg, F1, acc, recall, spe, pre))
