...
from models.baseline.base_files.base_model import BaseModel
from models.baseline.base_files.helper import initialize_weights
from itertools import chain

########################################################################################################################
# 版权所有：台州学院人工智能小组（2023.04.06）
# 代码修改人：Jiangxiong Fang
# 常用的模型有：A Deep Neural Network Architecture for Real-Time Semantic Segmentation
########################################################################################################################
...
class ENet(BaseModel):
    def __init__(self, n_channels=3, n_classes =1, freeze_bn=False, **_):
        super(ENet, self).__init__()
        self.initial = InitalBlock(n_channels)
        ...
        # Stage 6
        self.fullconv = nn.ConvTranspose2d(16, n_classes, kernel_size=3, padding=1,
                                           output_padding=1, stride=2, bias=False)
