...
class LiverContextNet(nn.Module):
    def __init__(self, in_ch=3, out_ch=1):
        super(LiverContextNet, self).__init__()

        n=1
        filters = [16*n, 32*n, 64*n, 128*n, 256*n, 512*n]
        self.encoder = EncoderNet(in_ch)
        self.mulAtt1 = MultiAttBlock(filters[0], filters[1])
        self.mulAtt2 = MultiAttBlock(filters[1], filters[2])
        self.mulAtt3 = MultiAttBlock(filters[2], filters[3])
        self.mulAtt4 = MultiAttBlock(filters[3], filters[4])

        # self.FE1 = FeaterExtra(filters[4], filters[1], filters[0])
        # self.FE2 = FeaterExtra(filters[4], filters[2], filters[1])
        # self.FE3 = FeaterExtra(filters[4], filters[3], filters[2])

        self.FE1 = FeaterExtra(filters[4])
        self.FE2 = FeaterExtra(filters[4])
        self.FE3 = FeaterExtra(filters[4])

        self.fa1 = FeatureAggregration(filters[3], filters[3], filters[4])
        self.fa2 = FeatureAggregration(filters[2], filters[2], filters[4])
        self.fa3 = FeatureAggregration(filters[1], filters[1], filters[4])

        self.fuse4 = Featureconn(filters[4],filters[4])
    ...
    def forward(self, x):
        ...
        top_layer1 = self.mulAtt1(layer1, layer2)
        top_layer2 = self.mulAtt2(layer2, layer3)
        top_layer3 = self.mulAtt3(layer3, layer4)
        top_layer4 = self.mulAtt4(layer4, layer5)

        down_layer1 = self.FE1(layer5)
        down_layer2 = self.FE2(layer5)
        down_layer3 = self.FE3(layer5)

        out_layer5 = self.conv_layer5(layer5)
        out_decoder4 = self.fuse4(top_layer4, out_layer5)
        out_decoder4 = self.conv_layer4(out_decoder4)

        out_decoder3 = self.fa1(top_layer3, out_decoder4, down_layer3)
        out_decoder3 = self.conv_layer3(out_decoder3)

        out_decoder2 = self.fa2(top_layer2, out_decoder3, down_layer2)
        out_decoder2 = self.conv_layer2(out_decoder2)

        out_decoder1 = self.fa3(top_layer1, out_decoder2, down_layer1)
...
class LiverContextNetA(nn.Module):
    def __init__(self, in_ch=3, out_ch=1):
        super(LiverContextNetA, self).__init__()
        ...
        # self.fuse4 = Featureconn(filters[3],filters[4])
