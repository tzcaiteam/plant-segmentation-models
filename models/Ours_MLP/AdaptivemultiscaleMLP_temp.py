...
# from ResNetV2 import ChannelResNet as ResNetV2    D:\jht_code\plant-segmegmentation-models\models\Ours_MLP\ResNetV2.py
from models.Ours_MLP.ResNetV2 import ChannelResNet as ResNetV2
...
class UNet_MLP(nn.Module):
    def __init__(self, n_channels, n_classes, basic_chans = 16, bilinear=True):
        ...
        factor = 2 if bilinear else 1
        self.down4 = Down(self.basic_channel * 32, self.basic_channel * 64 // factor)
        # self.down4 = Down(self.basic_channel * 32, self.basic_channel * 64)
        ...
        # Multi-scale MLP
        # self.up1 = CrossGatingBlock(self.basic_channel * 32, self.basic_channel * 32, self.basic_channel * 32, [res1, res1], [8, 8], [4, 4], 0.1, upsample_y=False,
        ...
        self.up1 = AdaptiveCrossGatingBlock(self.basic_channel * 16, self.basic_channel * 16, self.basic_channel * 16,
                                             [res4, res4], 0.1, train_size=resolution)
        self.up2 = AdaptiveCrossGatingBlock(self.basic_channel * 8, self.basic_channel * 8, self.basic_channel * 8,
                                             [res3, res3], 0.1, train_size=resolution)
        self.up3 = AdaptiveCrossGatingBlock(self.basic_channel * 4, self.basic_channel * 4, self.basic_channel * 4,
                                             [res2, res2], 0.1, train_size=resolution)

        self.up4 = AdaptiveCrossGatingBlock(self.basic_channel * 2, self.basic_channel * 2, self.basic_channel * 2,
                                             [res1, res1], 0.1, train_size=resolution)

        # self.fuseup1 = FusionUp(self.basic_channel * 64, self.basic_channel * 32, bilinear)
        # self.fuseup2 = FusionUp(self.basic_channel * 32, self.basic_channel * 16, bilinear)
        # self.fuseup3 = FusionUp(self.basic_channel * 16, self.basic_channel * 8, bilinear)
    ...
    def forward(self, x):
        ...
        #     x = x.repeat(1,3,1,1)
        #
        # x5, features = self.hybrid_model(x)
        # x1 = self.inc(x)
        # x2 = features[2]
        # x3 = features[1]
        # x4 = features[0]
        ...
        x5_out1, x5_out2 = self.up1(x5_1, x5_2)  # 32 --> 32   CrossGatingBlock类实例化对象up调用CrossGatingBlock类中的方法forward，传入2个参数
        mfi_out_1 = torch.cat([x5_out1, x5_out2], dim=1)
        x = self.fuseup1(mfi_out_1, x4)

        x = self.fuseup2(x, x3) #3,256， 64,64 ；3,128,128,128
        x = self.fuseup3(x, x2) #3,128， 128,128 ；3,32,256,256
        x = self.fuseup4(x, x1)
        out = self.outc(x)
        return out
...
class UNet_MLP_mask_all(nn.Module):
    def __init__(self, n_channels, n_classes, basic_chans=8, bilinear=False):
...
if __name__ == '__main__':
    # net = UNet_2D(n_channels=3, n_classes=1, basic_chans=8)
    # net = UNet_MLP(n_channels=3, n_classes=1, basic_chans = 8)  # 网络实例化
    # RuntimeError: Given groups=1, weight of size [256, 512, 3, 3], expected input[2, 768, 64, 64] to have 512 channels, but got 768 channels instead

    # RuntimeError: Given groups=1, weight of size [512, 512, 1, 1], expected input[2, 256, 32, 32] to have 512 channels, but got 256 channels instead
    # net = UNet_MLP(n_channels=3, n_classes=1, basic_chans=16)

    # RuntimeError: Given groups=1, weight of size [384, 384, 1, 1], expected input[2, 256, 32, 32] to have 384 channels, but got 256 channels instead
    # net = UNet_MLP(n_channels=3, n_classes=1, basic_chans=12)

    net = UNet_MLP(n_channels=3, n_classes=1, basic_chans=8)

    # net = UNet_MLP_Reduce(n_channels=3, n_classes=1, basic_chans=8)
    # net = UNet_MLP_mask(n_channels=3, n_classes=1, basic_chans=8)
    # net = UNet_MLP_mask_all(n_channels=3, n_classes=1, basic_chans=8)  UNet_MLP_mask
    # net.cuda()
    x = torch.randn(2,3,512,512) # 与输入大小一致的张量
    # x.cuda()
    # print(x.shape)
    # print(x)
    out = net(x)
