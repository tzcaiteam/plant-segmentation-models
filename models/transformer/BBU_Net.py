...
class BBU_Net(nn.Module):
    def __init__(self, in_ch, num_class, reduce_size=8, block_list='234'):
        ...
        if '1' in block_list:
            self.trans_1 = block_trans(256, num_blocks[-3], 256 // num_heads[-3], attn_drop=attn_drop,
                                       proj_drop=proj_drop, reduce_size=reduce_size, projection=projection,
                                       rel_pos=rel_pos)
        else:
        ...
        if '2' in block_list:
            self.trans_2 = block_trans(512, num_blocks[-2], 512 // num_heads[-2], attn_drop=attn_drop,
                                       proj_drop=proj_drop, reduce_size=reduce_size, projection=projection,
                                       rel_pos=rel_pos)
        else:
        ...
        if '3' in block_list:
            self.trans_3 = block_trans(1024, num_blocks[-1], 1024 // num_heads[-1], attn_drop=attn_drop,
                                       proj_drop=proj_drop, reduce_size=reduce_size, projection=projection,
                                       rel_pos=rel_pos)
        else:
