...
#         self.W_m = BN_Conv2d(out_ch, in_up, 1, 1, 0, bias=False)
#         self.W_g = BN_Conv2d(in_up, in_up, 3, 1, 1, bias=False)
#         self.m_psi = nn.Sequential(
#             nn.Conv2d(in_up, 1, kernel_size=1, stride=1, padding=0, bias=True),
#             nn.BatchNorm2d(1),
#             nn.Sigmoid()
#         )
#
...
class MBFUNet_A(nn.Module):

    def __init__(self, in_nc=3, config=[2, 2, 2, 2, 2, 2, 2], dim=64, drop_path_rate=0.4, input_resolution=256):
        super(MBFUNet_A, self).__init__()
        self.config = config
        self.dim = dim
        self.head_dim = 8
        self.window_size = 8
...
class MBFUNet_C(nn.Module):

    def __init__(self, in_nc=3, config=[2, 2, 2, 2, 2, 2, 2], dim=64, drop_path_rate=0.4, input_resolution=256):
        super(MBFUNet_C, self).__init__()
        self.config = config
        self.dim = dim
        self.head_dim = 8
        self.window_size = 8
...
class SU_Net(nn.Module):
    ...
    def __init__(self, in_nc=3, config=[2, 2, 2, 2, 2, 2, 2], dim=64, drop_path_rate=0.4, input_resolution=256):
        ...
        self.m_down1 = [nn.Conv2d(dim, 2 * dim)]
        self.down = [nn.MaxPool2d(kernel_size=2, stride=2)]
        self.m_down2 = [nn.Conv2d(2 * dim, 4 * dim)]
        self.up = [nn.MaxPool2d(scale_factor=2)]
...
# class MBFUNet(nn.Module):
#
#     def __init__(self, in_nc=3, config=[2, 2, 2, 2, 2, 2, 2], dim=64, drop_path_rate=0.4, input_resolution=256):
#         super(MBFUNet, self).__init__()
#         self.config = config
#         self.dim = dim
#         self.head_dim = 8
#         self.window_size = 8
