...
class BAU_NetAB_small(nn.Module):
    """
    ...
    def __init__(self, in_ch=1, out_ch=3):
        super(BAU_NetAB_small, self).__init__()
...
class BAU_NetAB_INCEPTION(nn.Module):
    """
    UNet - Unet with down-upsampling operation
    Paper : https://arxiv.org/abs/1505.04597
    """
    ...
    def __init__(self, in_ch=1, out_ch=3):
        super(BAU_NetAB_INCEPTION, self).__init__()
...
class BAU_NetC(nn.Module):
    """
    UNet - Basic Implementation
    Paper : https://arxiv.org/abs/1505.04597
    """
    def __init__(self, in_ch=3, out_ch=1):
        super(BAU_NetC, self).__init__()
        ...
        self.inconv0 = conv_block(in_ch, filters[0])
        self.inconv1 = conv_block(filters[0], filters[1])
        self.inconv2 = conv_block(filters[1], filters[2])
        self.inconv3 = conv_block(filters[2], filters[3])
        self.inconv4 = conv_block(filters[3], filters[4])
        ...
        self.Up4 = up_conv(filters[3], filters[2], 2)
        self.Uup4 = up_conv(filters[4], filters[2], 4)
        self.Up_conv4 = conv_block(192, filters[2])

        self.Up3 = up_conv(filters[2], filters[1], 2)
        self.Uup3 = up_conv(filters[4], filters[1], 8)
        self.Up_conv3 = conv_block(96, filters[1])

        self.Up2 = up_conv(filters[1], filters[0], 2)
        self.Uup2 = up_conv(filters[4], filters[0], 16)
        self.Up_conv2 = conv_block(48, filters[0])
    ...
    def forward(self, x):
        ...
        d4 = self.Up4(d5)
        f4 = self.Uup4(e5)
        d4 = torch.cat((e3, d4, f4), dim=1)
        d4 = self.Up_conv4(d4)

        d3 = self.Up3(d4)
        f3 = self.Uup3(e5)
        d3 = torch.cat((e2, d3, f3), dim=1)
        d3 = self.Up_conv3(d3)

        d2 = self.Up2(d3)
        f2 = self.Uup2(e5)
        d2 = torch.cat((e1, d2, f2), dim=1)
        d2 = self.Up_conv2(d2)
...
class BAU_Net_v0(nn.Module):
    ...
    def __init__(self, in_ch=3, out_ch=1):
        super(BAU_Net_v0, self).__init__()
...
class BAU_Net_PLUS(nn.Module):
    ...
    def __init__(self, in_ch=3, out_ch=1):
        super(BAU_Net_PLUS, self).__init__()
...
class BAU_Net_v1(nn.Module):
    ...
    def __init__(self, in_ch=3, out_ch=1):
        super(BAU_Net_v1, self).__init__()
...
class BAU_Net_incep(nn.Module):
    ...
    def __init__(self, in_ch=3, out_ch=1):
        super(BAU_Net_incep, self).__init__()
...
#     return model_test
#
# net = model_unet(BAU_Net_PLUS,1,3)
# net.apply(init)
#
# # # 输出数据维度检查
# # net = net.cuda()
