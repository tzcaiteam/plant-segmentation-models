...
# model_name：模型名称，应该与文件的模型名称完全一致,AdaptivemultiscaleMLP
# model_type：
...
def parse_args():
    ...
    parser.add_argument('--root', type=str, default=r'D:\jht_datasets\pepper2\pepper\bingdubing\\',
                        help='dataset name')
    parser.add_argument('--model_name', type=str, default='AdaptivemultiscaleMLP_Ablation',
                        help='model name: (default: arch+timestamp)')
    parser.add_argument('--model_type', default='AdaptivemultiscaleMLP_Ablation',type=str,
                        help='dataset name')
    parser.add_argument('--filesname', default='pepper_bingdu',type=str,
                        help='dataset name')
...
def main():
    ...
    if config['loss'] == 'BCEWithLogitsLoss':
        ...
        criterion = nn.BCELoss().cuda()  # 将loss计算放在gpu上
    else:
    ...
    ############################################################################################################
    # UNet_2D : the backbone network like the UNet with half a number of channels
    # UNet_MLP: the MLP-based network, and the decoder consists of the multiscale MLP-based blocks
    # UNet_MLP_Reduce: Adaptive Multi-scale gate for the features
    # UNet_MLP_mask: incuding one mask
    # UNet_MLP_mask_all
    ############################################################################################################

    if config['model_type'] == 'AdaptivemultiscaleMLP_Ablation':
        # abl_stores = ['UNet_2D', 'UNet_MLP', 'UNet_MLP_Reduce', 'UNet_MLP_mask', 'UNet_MLP_mask_all']
        abl_stores = ['UNet_2D', 'UNet_MLP', 'UNet_MLP_Reduce', 'UNet_MLP_mask', 'UNet_MLP_mask_all']
    # elif config['model_type'] == 'AdaptivemultiscaleMLP':
    #     abl_stores = ['UNet_2D', 'UNet_MLP', 'UNet_MLP_Reduce', 'UNet_MLP_mask', 'UNet_MLP_mask_all']

    for each_model in abl_stores:
        ...
        ##### build output path
        # model_output_path = '../model_output/%s/%s/%s/' % (config['filesname'], config['model_type'], config['model_type'])
        ...
        # Net = Net.to(device)
        model = model.cuda()   # 网络实例化之后，将网络放在gpu上
