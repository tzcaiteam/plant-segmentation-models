...
def train(config, train_loader, model, criterion, optimizer):
    ...
    for input, target, _ in train_loader:
        ...
        if config['deep_supervision']:
            ...
            output = model(input)  # rgb图经过网络后输出mask
            loss = criterion(output, target)
            iou = iou_score(output, target)

        # compute gradient and do optimizing step
        optimizer.zero_grad()  # 每个可迭代对象（数据总数/batch 个）都进行梯度清零  == module.zero_grad()
        loss.backward()  # 训练阶段反向传递loss, 对required=True的张量自动求导
        optimizer.step()  # 优化器进行参数更新 求导寻找最小loss
    ...
    with torch.no_grad():
        ...
        for input, target, _ in val_loader:
            ...
            if config['deep_supervision']:
                ...
                output = model(input)
                loss = criterion(output, target)    # 输入rgb图像经过网络输出mask掩码，进行评估指标的计算
                iou = iou_score(output, target)
...
# model_type：模型类型，对应于6个类型：Baseline，MLP，Transformer,Ours_MLP, Ours_transformer,Ours_CNN
# 其中：  Baseline包括： UNet, Attention UNet(), NestedUNet,Se_PPP_ResUNet,mobile_seg
#        MLP：
#        Transformer：
#        Ours_CNN: SKU_Net, BoundaryAware_UNet, SHAU_Net
# epochs:         训练次数
# batch_size：    批次，默认60次
...
def parse_args():
    ...
    # parser.add_argument('--root', type=str, default='/home/fang/datasets/pepper/zaoyibing/',
    #                     help='dataset name')
    parser.add_argument('--root', type=str, default=r'D:\jht_datasets\pepper2\pepper\hebanbing', # ***********
                        help='dataset name')

    parser.add_argument('--model_name', type=str, default='CM_MLP',   # *******************
    ...
    #                     help='dataset name')
    parser.add_argument('--filesname', default='pepper_hebanbing',type=str,   # **************************
                        help='dataset name')
...
def main():
    ...
    if config['model_name'] is None:
        if config['deep_supervision']:
            config['model_name'] = '%s_%s' % (config['model_type'], config['model_name'])
        else:
            config['model_name'] = '%s_%s' % (config['model_type'], config['model_name'])

    ##### build output path
    model_output_path = '../model_output/%s/%s/%s/' % (config['filesname'], config['model_type'], model_name+'_lateral_map_3')
    ...
    if config['dataset_type'] == 'val_and_train':
        ...
        val_loader = torch.utils.data.DataLoader(
            ...
            shuffle=False,
            num_workers=config['num_workers'],
            drop_last=False)  # 返回一个可迭代对象 含有数据 总数/batch_size 单元  每个单元含有batch_size个数据
    ...
    #########################################################################
    # 1. U-Net the basic UNet
    # 2. AttU_Net Attention UNet
    # 3. R2U_Net Recurrent Residual CNN
    # 4. NestedUNet
    # 5. Se_PPP_ResUNet
    # 6. R2AttU_Net Residual Recuurent Block with attention Unet
    # 7. UNet_MLP_mask_all
    # 8. CM_MLP
    ##########################################################################
    if config['model_type'] == 'Baseline':
        if model_name =='UNet':
            ...
            model = net(n_channels=3, n_classes=1, deep_supervision=deep_supervision)
        elif model_name == 'NestedUNet': # UNet++
            from models.baseline.BasicUNet import NestedUNet as net
            model = net(n_channels=3, n_classes=1, deep_supervision=deep_supervision)
        elif model_name == 'Se_PPP_ResUNet':
            from models.baseline.Se_PPP_ResUNet import Se_PPP_ResUNet as net
            model = net(n_channels=3, n_classes=1, deep_supervision=deep_supervision)
        elif model_name == 'R2AttU_Net':
            from models.baseline.BasicUNet import R2AttU_Net as net
            model = net(n_channels=3, n_classes=1, deep_supervision=deep_supervision)
        elif model_name == 'SegNet':
            from models.baseline.SegNet import FeatureResNet, SegResNet
            pretrained_net = FeatureResNet()
            config['deep_supervision'] = False
            pretrained_net.load_state_dict(torch.load('../models/baseline/base_files/resnet34.pth'))
            model = SegResNet(1, pretrained_net)
        elif model_name == 'ENet':
            from models.baseline.ENet import ENet as net
            config['deep_supervision'] = False
            model = net(n_channels=3, n_classes=1, deep_supervision=deep_supervision)
        elif model_name == 'FCN-VGG16':
            from models.baseline.FCN import FCNs as FCN_VGG16
            from models.baseline.FCN import VGGNet
            vgg_model = VGGNet()
            model = FCN_VGG16(pretrained_net=vgg_model,n_class = num_class)
    elif config['model_type'] == 'MLP':
        if model_name == 'UNeXt':
            from models.MLP.UNeXt import UNext as net
            config['deep_supervision'] = False
            model = net(n_channels=3, n_classes=1, deep_supervision=deep_supervision)
        elif model_name == 'CM_MLP':
            from models.MLP.CM_MLP import CM_MLP as net
            config['deep_supervision'] = False
            model = net(n_channels=3, n_classes=1)
        ...
        pretrained = False
        if model_name == 'UTNet':
            from models.transformer.utnet import UTNet as net
            config['deep_supervision'] = False
            model = net(3, base_chan, num_class, reduce_size=reduce_size,
        ...
        # elif model_name == 'TResUnet':
        #     # from models.transformer.TResUNet import TResUnet as net
        #     model = net()
        #     config['deep_supervision'] = False

    elif config['model_type'] == 'Ours_MLP':
        if model_name == 'MLPGLUNet':
            from models.Ours_MLP.MLPGLUNet import MLPGLUNet
            model = MLPGLUNet(config['num_classes'],config['input_channels'],
                      config['deep_supervision'])
        elif model_name == 'DoubleMLPUNet_DownAtt':
        ...
        elif model_name == 'UNet_MLP_mask_all':
            from models.Ours_MLP.AdaptivemultiscaleMLP import UNet_MLP_mask_all
            model = UNet_MLP_mask_all(config['input_channels'], config['num_classes'])

    elif config['model_type'] == 'Ours_CNN':
        if model_name == 'SKU_Net':
            from models.Ours_CNN.AdaAttU_Net import SKU_Net as net
            model = net(n_channels=3, n_classes=1,deep_supervision = deep_supervision)
        elif model_name == 'BoundaryAware_UNet':
            from models.Ours_CNN.AdaAttU_Net import BoundaryAware_UNet as net
            model = net(n_channels=3, n_classes=1, deep_supervision=deep_supervision)
        elif model_name == 'SHAU_Net':
            from models.Ours_CNN.AdaAttU_Net import SHAU_Net as net
            model = net(n_channels=3, n_classes=1,deep_supervision = deep_supervision)
        elif model_name == 'GCHA_Net':
            from models.Ours_CNN.GCHA_Net import GCHA_Net as net
            config['deep_supervision'] = False
            model = net(n_channels=3, n_classes=1)
