...
def parse_args():
    ...
    # parser.add_argument('--name', default='Ablation_UNet_MLP_batchsize2',
    #                     help='model name')  # ******************************
    parser.add_argument('--deep_supervision', default=False, type = str2bool)
    # parser.add_argument('--model_path', default='/home/felicia/iterative_pepper/plant-segmegmentation-models/model_output/pepper_bingdu/Iteration/',
    #                     help='model path')
    parser.add_argument('--model_path', default=r'D:\jht_code\plant-segmegmentation-models\model_output\pepper_hebanbing\MLP',
                        help='model path') # ***********************************************

    # parser.add_argument('--valResult_path', default='/home/felicia/iterative_pepper/plant-segmegmentation-models/val_output/pepper_bingdu/Iteration/',
    #                     help='model path')
    parser.add_argument('--valResult_path', default=r'D:\jht_code\plant-segmegmentation-models\val_output\pepper_hebanbing\MLP',
                        help='model path') # ***********************************************
    ...
    # parser.add_argument('--test_path', default='/home/felicia/datasets/pepper/bingdu/Test/',     #image  label
    #                     help='test path of the testing iamges')
    parser.add_argument('--test_path', default=r'D:\jht_datasets\pepper2\pepper\hebanbing\Test\\',     #image  label
                        help='test path of the testing iamges') # ***********************************
...
def main():
    ...
    for each_model in model_stores:
        ...
        path = args.model_path +'\\' + each_model +'_lateral_map_3'+ '/config.yml'
        ...
        if each_model == 'UNet_2D':
            from models.Ours_MLP.AdaptivemultiscaleMLP import UNet_2D as net
            model = net(n_channels=3, n_classes=1, basic_chans = 8)

        elif each_model == 'UNet_MLP':
            from models.Ours_MLP.AdaptivemultiscaleMLP import UNet_MLP as net
            model = net(n_channels=3, n_classes=1, basic_chans = 8)
        elif each_model == 'UNet_MLP_Reduce':
            from models.Ours_MLP.AdaptivemultiscaleMLP import UNet_MLP_Reduce as net
            model = net(n_channels=3, n_classes=1, basic_chans = 8)
        elif each_model == 'UNet_MLP_mask':
            from models.Ours_MLP.AdaptivemultiscaleMLP import UNet_MLP_mask as net
            model = net(n_channels=3, n_classes=1, basic_chans = 8)
        elif each_model == 'UNet_MLP_mask_all':
            from models.Ours_MLP.AdaptivemultiscaleMLP import UNet_MLP_mask_all as net
            model = net(config['input_channels'], config['num_classes'])
            ...
            from models.Ours_MLP.MLPGLUNet import MLPGLUNet
            model = MLPGLUNet(config['num_classes'])
        elif each_model == 'FCN-VGG16':
            from models.baseline.FCN import FCNs as FCN_VGG16
            from models.baseline.FCN import VGGNet
            VGG16 = VGGNet()
            model = FCN_VGG16(pretrained_net=VGG16, n_class=config['num_classes'])
        elif each_model == 'CM_MLP':
        ...
        model.load_state_dict(torch.load(args.model_path + '\\' + each_model +'_lateral_map_3'+ '/best_model.pth'), strict=False)
        model.eval()
            ...
            for input, target, meta in tqdm(val_loader, total=len(val_loader)):
                ...
                for i in range(len(output)):
                    for c in range(config['num_classes']):
                        ...
                        # save_path1 = args.test_path + args.name + '/' + str(c)
                        save_path1 = args.test_path + 'pepper_' + each_model + '_2023_5_11' + '/' + str(c)

                        # save_path = args.test_path + each_model + '/' + str(c) + '/' + meta['img_id'][i].split('/')[-1]
                        save_path = save_path1 + '/' + meta['img_id'][i].split('/')[-1]
                ...
                #     pr, rr, F1r, accr, recallr, sper,))     IoU	F1	accuracy	recall	SPE	pre
                print('Precision: %.4f, Recall: %.4f, F1: %.4f, Accuracy: %.4f, Precision: %.4f, Recall:  %.4f' % (
    ...
    # print("Hausdorff distance test: {0}".format(hausdorff_distance(X, Y, distance="cosine")))
        ...
        # val_output_path = os.path.join(args.valResult_path, args.name)
        val_output_path = os.path.join(args.valResult_path, 'pepper_' + each_model + '_lateral_map_3')  # ******************************
        ...
        print('IoU: %.4f, F1: %.4f, accuracy: %.4f, recall: %.4f, SPE: %.4f, pre: %.4f, dice: %.4f' % (avg_meters['iou'].avg, F1, acc, recall, spe, pre, avg_meters['dice'].avg))
